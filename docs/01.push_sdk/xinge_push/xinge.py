...
class XingeHelper(object):
    ...
    def Request(cls, path, params):
        ...
        if cls.HTTP_METHOD == 'GET':
            httpClient.request(cls.HTTP_METHOD, ('%s?%s' % (path, urlencode(params))), headers=cls.HTTP_HEADERS)
        elif cls.HTTP_METHOD == 'POST':
            httpClient.request(cls.HTTP_METHOD, path, urlencode(params), headers=cls.HTTP_HEADERS)
        else:
            # invalid method
